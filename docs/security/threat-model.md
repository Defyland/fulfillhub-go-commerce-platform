...
| Operator actions | Unauthorized cancellation or replay | Operations bearer token, requested actor metadata, audit logs |
...
- Operations callers use `Authorization: Bearer ops-token` in the local slice.
...
- The local operations token is intentionally static; production needs signed JWT validation and role claims.
